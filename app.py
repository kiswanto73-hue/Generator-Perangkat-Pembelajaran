...
with tab_jurnal:
    ...
    if submit_jurnal:
        if not j_materi:
            ...
            st.session_state.jurnal_rows.append({
    ...
    if st.session_state.jurnal_rows:
        ...
        st.dataframe(st.session_state.jurnal_rows, use_container_width=True, hide_index=True)

        cja, cjb = st.columns(2)
        with cja:
            if st.button("🗑️ Hapus Catatan Terakhir", key="btn_hapus_jurnal"):
                st.session_state.jurnal_rows.pop()
                st.rerun()
        with cjb:
...
with tab_absen:
    ...
    if submit_siswa:
        if not s_nama:
            ...
            st.session_state.siswa_rows.append({"Nama": s_nama, "L/P": s_jk, "NISN": s_nisn})
    ...
    if st.button("📥 Impor dari Daftar Tempel", key="btn_impor_siswa"):
        ...
        for n in nama_list:
            st.session_state.siswa_rows.append({"Nama": n, "L/P": "-", "NISN": ""})
            ...
            if baris_siswa:
                ...
                if st.button("➕ Tambahkan Semua ke Data Siswa", key="btn_impor_siswa_file"):
                    for r in baris_siswa:
                        ...
                        if not nama_:
                            continue
                        st.session_state.siswa_rows.append({
                            "Nama": nama_,
    ...
    if st.session_state.siswa_rows:
        ...
        st.dataframe(st.session_state.siswa_rows, use_container_width=True, hide_index=True)

        sca, scb = st.columns(2)
        with sca:
            if st.button("🗑️ Hapus Siswa Terakhir", key="btn_hapus_siswa"):
                st.session_state.siswa_rows.pop()
                st.rerun()
        with scb:
